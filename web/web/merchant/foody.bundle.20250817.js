...
/*! Foody Bundle JS — 2025-08-17 fix2
 * - Tabs single-active guard
 * - Auth dedupe guard
 * - Create Offer UX:
 *   · discount chips placed RIGHT AFTER base price
 *   · discount% input (no rounding UI), bi-directional sync with final price
 *   · label text -> "Срок действия оффера"
 *   · nice select for category
 *   · FilePond image upload like new.html (preview, label, 5MB)
 */
(function(){
  ...
  function bindTabs(){
    ...
    function activate(name){ panes.forEach(p=>p.classList.toggle('active',p.id===name)); tabs.forEach(b=>b.classList.toggle('active',b.dataset.tab===name)); try{history.replaceState(null,'','#'+name);}catch(_){}};
    tabs.forEach(b=> b.addEventListener('click',()=>{ const name=b.dataset.tab; if(name&&document.getElementById(name)) activate(name); }));
    const actives=panes.filter(p=>p.classList.contains('active')); if(actives.length!==1){ const first=document.querySelector('#tabs .seg-btn.active')||tabs[0]; activate((first&&first.dataset.tab)||'dashboard'); }
  }
  ...
  function enhanceCreateOffer(){
    ...
    // 1) Move DISCOUNT controls right after base price
    if (base && chipsWrap){
      const baseBox = base.closest('label') || base.parentElement;
      if (baseBox && baseBox.parentElement){
        baseBox.parentElement.insertBefore(chipsWrap, baseBox.nextSibling);
      }
    }

    // 2) Ensure discount% input exists and is placed right after chips
    ...
    if (!disc){
      ...
      else (form.firstChild ? form.insertBefore(row, form.firstChild.nextSibling) : form.appendChild(row));
      disc = row.querySelector('#discountPercent');
    }

    // 3) Rename label text for expires
    if (expires){
      const lbl = expires.closest('label');
      if (lbl){
        setLabelText(lbl, 'Срок действия оффера');
      }
    }

    // 4) Ensure bestBefore exists (before expires)
    ...
    // 5) Default expires: closing today if possible, else +2h
    if (expires && !expires.value){
      const close = computeClosingTime();
      const now = new Date();
      const def = (close && close.getTime()>now.getTime()) ? close : new Date(now.getTime()+2*60*60*1000);
      expires.value = toLocalInputValue(def);
    }

    // 6) Events for chips expire
    ...
    try{
      ...
      if (input && hidden && typeof FilePond !== 'undefined'){
        ...
        pond.on('addfile', async (err, item) => {
          if (err) return;
          try{ hidden.value = (await uploadImage(item.file)) || ''; }catch(_){ hidden.value = ''; }
        });
    ...
    function computeClosingTime(){
      const to=document.getElementById('profile_work_to'); const now=new Date();
    ...
    function setLabelText(labelEl, text){
      // replace only text before first element
